...
from models.egovlp.base import BaseModel
from models.egovlp.video_transformer import SpaceTimeTransformer
from models.egovlp.util import state_dict_data_parallel_fix
    ...
    def __init__(self, video_params, text_params, projection_dim=256, load_checkpoint=None, projection="minimal", load_temporal_fix="zeros"):
        ...
        if video_params["model"] == "SpaceTimeTransformer":
            ...
            if arch_config == "base_patch16_224":
                ...
                vit_model = torch.load(video_params["vit_ckpt"], map_location="cpu")
        ...
        if load_checkpoint not in ["", None]:
            ...
            state_dict = torch.load(load_checkpoint, map_location="cuda", weights_only=True)
    ...
    def _inflate_positional_embeds(self, new_state_dict):
        ...
        if "video_model.temporal_embed" in new_state_dict and "video_model.temporal_embed" in curr_keys:
            ...
            if load_num_frames != curr_num_frames:
                if load_num_frames > curr_num_frames:
                    print(f'### loaded {self.video_params["model"]} model has MORE frames than current...' f"### loading weights, filling in the extras via {self.load_temporal_fix}")
                    new_temporal_embed = load_temporal_embed[:, :curr_num_frames, :]
                else:
                    print(f'### loaded {self.video_params["model"]} model has FEWER frames than current...' f"### loading weights, filling in the extras via {self.load_temporal_fix}")
